function [ft_struct,rejected_comps] = RFT_IClabel(ft_struct,eeglab_template_file,numComp,threshold)
...
try
    eeglab_struct = eeglab_template.eeglab_template; %eeglab_template.eeglab_template;
catch 
...
%%%% CALL IClabel %%%%
addpath('C:\Users\tnguyen\OneDrive - Fondazione Istituto Italiano Tecnologia\Documenti\MATLAB/toolboxes/eeglab2022.0');
eeglab; close; % add paths to EEGLAB

% Compute ICA
assert(exist('numComp','var') && ~isempty(numComp) , 'You must specify how many ICs to extract');
eeglab_struct = pop_runica(eeglab_struct, 'icatype', 'runica','pca',numComp);  
eeglab_struct = iclabel(eeglab_struct);

% Plot ICs and max probability of category
pop_viewprops(eeglab_struct, 0, 1:numComp, [], [], [], 'IClabel') % for component properties

% Detect ICs of the categories of interest
...
rmpath(genpath('C:\Users\tnguyen\OneDrive - Fondazione Istituto Italiano Tecnologia\Documenti\MATLAB\toolboxes\eeglab2022.0'));
end
