function [all_erp_all_midi] = Trinh_MiniTrialMaker(data,midi_names,code_corresponding_to_midi,n_offsets,prestim)
%
...
for i = 1:length(code_corresponding_to_midi)
    ...
    for ii = 1:length(instances_of_same_midi) % loop into same exposure of 1 song
        ...
        try
            note_onsets_corrected = round((note_onsets* data.fsample) + EEG_of_one_midi.sampleinfo(1) + (prestim* data.fsample));
        catch
            note_onsets_corrected = round(note_onsets* data.fsample);
        end
        cfg                              = [];
